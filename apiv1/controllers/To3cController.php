				...
				switch (strtolower($key)) {
					...
					case 'pair':
						...
						elseif (!empty($pair) && (strpos($pair, '_') === false)) {
							...
							else { $ok = false; Yii::trace('ERROR: Base (BTC, USDT, BUSD) not found in pair='.$pair); break 2; }
						}
...
// ==========
		...
		if ($ok) {
			try {
				$discordLogMessages = [];
				$sql = "SELECT id, sig_code, sig_discordlogchanid, sig_discordlogdelaychanid, sig_discordmessage, sig_discorddelayminutes "
							." FROM `signal`"
							." WHERE (sig_discordlogchanid > '' OR sig_discordlogdelaychanid > '') AND sig_discordmessage > '' AND sig_discorddelayminutes IS NOT NULL AND sig_active=1 AND sig_deletedat=0";
				$rows = GeneralHelper::runSql($sql);
				if (!empty($rows)) {
					foreach($rows as $nr => $row) if (is_numeric($nr)) {
						$discordLogMessages[ $row['sig_code'] ] = [
							'channelid' => $row['sig_discordlogchanid'],
							'channeldelayid' => $row['sig_discordlogdelaychanid'],
							'message' => $row['sig_discordmessage'],
							'delaymins' => $row['sig_discorddelayminutes']
						];
					}
				}
				Yii::trace('** index get discordLogMessages: '.print_r($discordLogMessages, true));
			} catch (\Exception $e) {
				$msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
				Yii::error('** index get discordLogMessages ERROR msg='.$msg);
			}
...
// ================
          ...
          'message'  => '['.$data['action'].']: ' . ($ok ? $result.' records updated' : $msg)
				...
				try {
					$discordLogChannelID = $discordLogDelayChannelID = $discordLogMessage = $discordLogDone = $discordLogAt = '';
					Yii::trace('** index LogToDiscord 1 action='.$data['action'].' discordLogMessages: '.print_r($discordLogMessages, true));
					if (!empty($discordLogMessages[ $data['action'] ])) {
						$discordLog = $discordLogMessages[ $data['action'] ];
						$discordLogMessage = $discordLog['message'];
						if (strpos($discordLogMessage, '{logtime}') !== false) $discordLogMessage = str_replace("{logtime}", date('d-M-y H:i')." CET", $discordLogMessage);
						if (strpos($discordLogMessage, '{pair}')    !== false) $discordLogMessage = str_replace("{pair}",    $data['pair'],     $discordLogMessage);
						$discordLogChannelID = $discordLog['channelid'];
						$discordLogDelayChannelID = $discordLog['channeldelayid'];
						if (!empty($discordLogChannelID)) {
							$result = json_encode( GeneralHelper::sendMessageToDiscordCategory($discordLogChannelID, $discordLogMessage) );
							$discordLogDone = date('ymd-His').'->'.$discordLogChannelID.'='.(empty($result) ? "ok" : ((strpos($result,'error') === false) ? "Ok" : $result));
						}
						if (!empty($discordLogDelayChannelID)) {
							$discordLogAt = date("Y-m-d H:i:s", strtotime("+" . $discordLog['delaymins'] . " minutes"));
						}
						Yii::trace('** index LogToDiscord 1 ChannelID='.$discordLogChannelID.' DelayChannelID='.$discordLogDelayChannelID.' discordLogAt='.$discordLogAt.' discordLogMessage='.$discordLogMessage.' discordLogDone='.$discordLogDone);
					}
				} catch (\Exception $e) {
          $msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
          Yii::error('** index error LogToDiscord 1 ERROR msg='.$msg);
        }
				...
				try {
					...
					$signallog->slg_discordlogchanid = $discordLogDelayChannelID;
					$signallog->slg_discordlogmessage = $discordLogMessage;
					$signallog->slg_discordtologat = $discordLogAt;
					$signallog->slg_discordlogdone = $discordLogDone;
					$signallog->slg_discordlogdelaydone = ''; // must be emptie
					...
					if (! ($saveResult=$signallog->save(false))) {
						$sigid = 0;
						Yii::error('** index error save signallog 1: '.print_r($saveResult, true));
					} else {
						$sigid = $signallog->id;
					}
				} catch (\Exception $e) {
					$msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
					Yii::error('** index error save signallog 1 msg='.$msg);
				}
...
// ----------
        ...
        try {
        /*  $discordLogChannelID = $discordLogMessage = $discordLogDone = $discordLogAt = '';
					Yii::trace('** index LogToDiscord 2 action='.$data['action'].' discordLogMessages: '.print_r($discordLogMessages, true));
          if (!empty($discordLogMessages[ $data['action'] ])) {
            $discordLog = $discordLogMessages[ $data['action'] ];
						$discordLogMessage = $discordLog['message'];
						if (strpos($discordLogMessage, '{logtime}') !== false) $discordLogMessage = str_replace("{logtime}", date('d-M-y H:i'), $discordLogMessage);
						if (strpos($discordLogMessage, '{pair}')    !== false) $discordLogMessage = str_replace("{pair}",    $data['pair'],     $discordLogMessage);
            $discordLogChannelID = $discordLog['channelid'];
            if ($discordLog['delaymins'] == 0) {
              $result = GeneralHelper::sendMessageToDiscordCategory($discordLogChannelID, $discordLogMessage);
              $discordLogAt = date("Y-m-d H:i:s");
              $discordLogDone = (empty($result) ? "ok" : json_encode($result));
            } else {
              $discordLogAt = date("Y-m-d H:i:s", strtotime("+" . $discordLog['delaymins'] . " minutes"));
            }
						Yii::trace('** index LogToDiscord 2 discordLogChannelID='.$discordLogChannelID.' discordLogAt='.$discordLogAt.' discordLogMessage='.$discordLogMessage.' discordLogDone='.$discordLogDone);
          }
        } catch (\Exception $e) {
          $msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
          Yii::error('** index error LogToDiscord 2 ERROR msg='.$msg);
        }*/
					...
					if (!empty($discordLogMessages[ $data['action'] ])) {
						...
						Yii::trace('** index LogToDiscord 2 ChannelID='.$discordLogChannelID.' DelayChannelID='.$discordLogDelayChannelID.' discordLogAt='.$discordLogAt.' discordLogMessage='.$discordLogMessage.' discordLogDone='.$discordLogDone);
					}
				} catch (\Exception $e) {
					$msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
					Yii::error('** index error LogToDiscord 2 ERROR msg='.$msg);
        ...
        try {
          ...
          $signallog->slg_senddata = json_encode($logsenddata);
					...
					$signallog->slg_discordtologat = $discordLogAt;
					$signallog->slg_discordlogdone = $discordLogDone;
					$signallog->slg_discordlogdelaydone = ''; // must be empty
          //$signallog->slg_message = '['.$data['action'].']: ' . (is_array($result) ? print_r($result,true) : $result.' records updated');
          $signallog->slg_createdat     = $signallog->slg_updatedat = time();
          $signallog->slgusr_created_id = $signallog->slgusr_updated_id = 1; // SystemLogUser  ToDo ===> does not accept - query gives null value  == DB set to maybe NULL ..
          $signallog->slg_createdt      = $signallog->slg_updatedt = date('Y-m-d H:i:s', time());
          ...
          if (! ($saveResult=$signallog->save(false))) {
            $sigid = 0;
            Yii::error('** index error save signallog 2: '.print_r($saveResult, true));
          } else {
            $sigid = $signallog->id;
          }
        } catch (\Exception $e) {
          $msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
          Yii::error('** index error save signallog 2 msg='.$msg);
        }
...
// ---------
				...
				if ($signalStartStop) {
					...
					$sql = "SELECT DISTINCT ubt.id as id, ubt.ubt_3cdealstartjson, bsg.id as bsg_id, sig.id as sig_id, sig.sig_code, sig.sig_3cactiontext, ubt.ubt_name, umb.umb_name"
								...
								."   AND ubt.ubtumb_id=umb.id AND umb.umb_active=1 AND umb.umb_deletedat=0"
								."   AND upy.upyumb_id=umb.id AND upy.upy_state='".Userpay::UPY_STATE_PAID."' AND (DATE(NOW()) BETWEEN upy.upy_startdate and upy.upy_enddate)"
								." ORDER BY umb.umb_name, ubt.ubt_name, ubt.id";
        ...
        try {
          /*$discordLogChannelID = $discordLogMessage = $discordLogDone = $discordLogAt = '';
					$action = (!empty($data['action']) ? $data['action'] : $data['signal']);
					Yii::trace('** index LogToDiscord 3 action='.$action.' data: '.print_r($data, true).' discordLogMessages: '.print_r($discordLogMessages, true));
          if (!empty($discordLogMessages[ $action ])) {
            $discordLog = $discordLogMessages[ $action ];
						$discordLogMessage = $discordLog['message'];
						if (strpos($discordLogMessage, '{logtime}') !== false) $discordLogMessage = str_replace("{logtime}", date('d-M-y H:i'), $discordLogMessage);
						if (strpos($discordLogMessage, '{pair}')    !== false) $discordLogMessage = str_replace("{pair}",    $data['pair'],     $discordLogMessage);
            $discordLogChannelID = $discordLog['channelid'];
            if ($discordLog['delaymins'] == 0) {
              ...
              $discordLogAt = date("Y-m-d H:i:s", strtotime("+" . $discordLog['delaymins'] . " minutes"));
						...
						Yii::trace('** index LogToDiscord 3 action='.$action.' discordLogChannelID='.$discordLogChannelID.' discordLogAt='.$discordLogAt.' discordLogMessage='.$discordLogMessage.' discordLogDone='.$discordLogDone);
          }*/
					...
					if (!empty($discordLogMessages[ $action ])) {
						...
						Yii::trace('** index LogToDiscord 3 ChannelID='.$discordLogChannelID.' DelayChannelID='.$discordLogDelayChannelID.' discordLogAt='.$discordLogAt.' discordLogMessage='.$discordLogMessage.' discordLogDone='.$discordLogDone);
					}
        } catch (\Exception $e) {
          $msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
          Yii::error('** index error LogToDiscord 3 action='.$action.' ERROR msg='.$msg);
        }
				...
				try {
					...
					$signallog->slg_createdt      = $signallog->slg_updatedt = date('Y-m-d H:i:s', time());
					if (! ($saveResult=$signallog->save(false))) {
						Yii::error('** index error save signallog 3: '.print_r($saveResult, true));
					}
				} catch (\Exception $e) {
					$msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
					Yii::error('** index error save signallog 3 msg='.$msg);
				}
			}
		}
	}
