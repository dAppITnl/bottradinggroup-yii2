...
$users = GeneralHelper::getUsersWithUserlevel(BackendUser::USR_SITELEVEL_HFTMEMBER);
// \yii\helpers\ArrayHelper::map(hftadmin\models\User::find()->all(), 'id', "CONCAT(IFNULL(usr_firstname,''),' ',IFNULL(usr_lastname,''),' (',username,')')");
$cryptoAddresses = \yii\helpers\ArrayHelper::map(backend\models\Cryptoaddress::find()->all(), 'id', 'cad_name');
  ...
  <div class="">
    ...
    <p>
      ...
      <?= $form->field($model, 'upy_name')->textInput(['maxlength' => true]) ?>
			...
			<!-- attribute upyusr_id -->
			<?= $form->field($model, 'upyusr_id')->dropDownList($users,
		  // \yii\helpers\ArrayHelper::map(hftadmin\models\User::find()->all(), 'id', "CONCAT(IFNULL(usr_firstname,''),' ',IFNULL(usr_lastname,''),' (',username,')')"),
		   [
		  ...
		  <?= $form->field($model, 'upyumb_id')->dropDownList( //$usermembers,
		    \yii\helpers\ArrayHelper::map(hftadmin\models\Usermember::find()->where(['umb_deletedat'=>0])->all(), 'id', 'umb_name'),
		    [
		      'prompt' => Yii::t('cruds', 'Select'),
      ...
      <?= $form->field($model, 'upy_state')->dropDownList($userpayStates,
        ...
        [
          ...
          //'disabled' => (isset($relAttributes) && isset($relAttributes['upy_state'])),
    		...
    		$payCategories,  //\yii\helpers\ArrayHelper::map(hftadmin\models\Category::find()->all(), 'id', 'id'),
    		[
        	'prompt' => Yii::t('cruds', 'Select'),
      ...
      <?= $form->field($model, 'upy_percode')->dropDownList(
        $pricelistPeriods, //hftadmin\models\Pricelist::optsprlpercode()
        [
          'prompt' => Yii::t('cruds', 'Select'),
        ]
      ); ?>
      ...
      <?= $form->field($model, 'upyprl_id')->dropDownList(
        \yii\helpers\ArrayHelper::map(hftadmin\models\Pricelist::find()->all(), 'id', 'prl_name'),
        [
          'prompt' => Yii::t('cruds', 'Select'),
      ...
      <?= $form->field($model, 'upysym_pay_id')->dropDownList(
        \yii\helpers\ArrayHelper::map(hftadmin\models\Symbol::find()->all(), 'id', 'sym_code'),
        [
          'prompt' => Yii::t('cruds', 'Select'),
      ...
      <?= $form->field($model, 'upy_fromaccount')->textInput(['maxlength' => true]) ?>
		  ...
		  <?= $form->field($model, 'upysym_crypto_id')->dropDownList(
		   \yii\helpers\ArrayHelper::map(hftadmin\models\Symbol::find()->all(), 'id', 'id'),
		   [
		       'prompt' => Yii::t('cruds', 'Select'),
      ...
      <?= $form->field($model, 'upy_cryptoamount')->textInput(['type' => 'number', 'step' => 'any']) ?>
		  ...
		  <?= $form->field($model, 'upycad_to_id')->dropDownList($cryptoAddresses,
		   //	\yii\helpers\ArrayHelper::map(Cryptoaddress::find()->all(), 'id', 'cad_name'),
		   [
		     'prompt' => Yii::t('cruds', 'Select'),
