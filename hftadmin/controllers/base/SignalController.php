...
use hftadmin\models\SignalSearch;
  ...
  public function behaviors()
  {
    ...
    return [
    	'access' => [
            ...
            'roles' => ['hftadminSignalFull'],
          ],
    			[
    				'allow' => true,
            'actions' => ['index', 'view'],
            'roles' => ['hftadminSignalView'],
          ],
    			[
    				'allow' => true,
            'actions' => ['update', 'create', 'delete'],
            'roles' => ['hftadminSignalEdit'],
          ],*/
          [
	...
	*/
    ...
    try {
      ...
      $ok = false;
			...
			$model->sigcat_ids = implode(',', Signal::HFTADMIN_CATEGORIES_SIG);
			//$model->sigmbr_ids = implode(',', Signal::HFTADMIN_MEMBERSHIPS);
			$model->sig_runenabled = 1; // always enabled
			$model->sig_maxbots = 0; // unlimited
      if ($model->load($_POST)) {
	...
	*/
    ...
    try {
      ...
      if ($model->load($_POST)) {
				//$model->sigcat_ids = ((!empty($model->sigcat_ids) && is_array($model->sigcat_ids)) ? implode(',', $model->sigcat_ids) : '');
				if (empty($model->sigcat_ids)) $model->sigcat_ids = implode(',', Signal::HFTADMIN_CATEGORIES_SIG);
				$model->sigmbr_ids = ((!empty($model->sigmbr_ids) && is_array($model->sigmbr_ids)) ? implode(',', $model->sigmbr_ids) : '');
				$model->sig_runenabled = 1; // always
				$model->sig_maxbots = 0; // unlimited
