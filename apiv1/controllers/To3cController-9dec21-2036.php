...
class To3cController extends Controller
{
    ...
    Yii::trace('** index pairs: '.print_r($pairs, true));
        ...
        try {
          $msg = '** Extreme Up/Downtrend: bot ' . ($signalstart==1 ? 'START' : 'STOP') . ' ontvangen; zie log: https://adminbtg.bottradinggroup.nl/signallog/view?id='.$sigid;
          $result = GeneralHelper::sendMessageToDiscordCategory('discord_premiumlogs', $msg);
          ...
          Yii::error('** index error send to discord 2 msg='.$msg);
				...
				if ($signalStartStop) {
					$sql = "SELECT ubt.id as id, ubt.ubt_3cdealstartjson, ubt.ubt_name, umb.umb_name, 0 as bsg_id, 0 as sig_id"
								." FROM (userbot ubt, usermember umb)"
					...
					$sql = "SELECT ubt.id as id, ubt.ubt_3cdealstartjson, bsg.id as bsg_id, sig.id as sig_id, sig.sig_code, sig.sig_3cactiontext, ubt.ubt_name, umb.umb_name"
								...
								."   AND (bsg.bsg_userstartstop=0 || (bsg.bsg_userstartstop=1 AND bsg.bsg_signalstartstop=1))" // user enabled check for alert has enabled sending signal
