Moralis.initialize("EywGHrSDegfAO5VKxY6oDcCL8g8B0z6VNU8vySd4"); // Botsignals Rinkeby testnet 7okt21 test
Moralis.serverURL = "https://zp3xpuvxaexw.moralishost.com:2053/server";

/* -- home -- */
function buildHomeComponent(user) {
  return `
    <h1>Welcome, ${user.attributes.username} to Botsignals</h>
  `;
}

function renderHome(user) {
  contentContainer.innerHTML = buildHomeComponent(user);
...
async function loginWithEmail(isSignUp) {
  ...
  try {
    if (isSignUp) {
      ...
      user.set("botsignalsActive", false);
...
function buildLoginComponent(isSignUp) {
  const btnSignUp = isSignUp
    ? ""
    : ` or <button type="button" id="btn-login-email-signup">Sign Up With Email</button>`;

  return `
    <div class="container login">
      <button id="btn-login-metamask">Login with MetaMask</button>
      <p>or Login with username / Signup with Email</p>
      <div id="frm-login">
        ...
        <button type="button" id="btn-login-email" type="button">Login</button>
        ${btnSignUp}
      </div>
    </div>
  `;
  ...
  return `
    <div class="container">
      <div>
        ...
        <button class="mt" type="button" id="btn-profile-cancel">Cancel</button>
      </div>
    </div>
  `;
}
...
function renderProfile(user) {
  ...
  document.getElementById("btn-profile-cancel").onclick = doEndProfile;
...
function renderHeader(user) {
  if (user) {
    ...
    appHeaderContainer.innerHTML = `
        <button id="btn-userProfile">Profile</button>
        <button id="btn-logout">Logout</button>
      `;
    document.getElementById("btn-userProfile").onclick = () => { renderProfile(user); }
