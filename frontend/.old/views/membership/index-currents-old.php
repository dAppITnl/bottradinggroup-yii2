	...
	<?php foreach($usermembersModel as $nr => $usermember) :
      ...
      <div class="col">
				<p><table class="table-100" border="1">
					...
					<tr><th><?= Yii::t('app', 'Last paid period') ?>: </th><td><?= GeneralHelper::showDateTime($usermember->umb_startdate,'dmY') . Yii::t('app', ' up to ') . GeneralHelper::showDateTime($usermember->umb_enddate,'dmY') ?></td></tr>
		...
		<?php if (count($usermember->userpays) > 0) :
					...
					<tr><th><?= Yii::t('app', 'Paid')       ?>: </th><td><?= GeneralHelper::showDateTime($userpay->upy_startdate,'dmy') . Yii::t('app', ' up to ') . GeneralHelper::showDateTime($userpay->upy_enddate,'dmy')
							.' ('. $userpay->upysymPay->sym_html . ' '. $userpay->upy_payamount
							.'; '. $pricelistPeriods[$userpay->upy_percode] .', ref='.$userpay->upy_payref.')' ?></td></tr>
			<?php endforeach;
					else: ?>
					<tr><th><?= Yii::t('app', 'Paid')       ?>: </th><td style="color:#ab0909;font-weight:600;"><?= Yii::t('app', 'Not paid') ?> <?= HTML::a( Yii::t('app', 'Pay'), ['/membership/pay', 'id'=>$usermember->id], ['class'=>'knop-2']) ?></td></tr>
		<?php endif; ?>
