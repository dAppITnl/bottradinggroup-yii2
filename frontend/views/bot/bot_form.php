...
$userpayData = Userpay::getCurrentUserpayOfUsermember( $usermemberModel->id );
$maxSignals = 1 * $userpayData['upy_maxsignals']; //$usermemberModel->umb_maxsignals;
$usedSignals = 1 * $signalCounts[$usermemberModel->id];
...
<div class="userbot">
  ...
  <div class="row">
    <div class="col">
      ...
      <tr><th><?= Yii::t('app', 'Membership')   ?>: </th><td valign="top"><span class="table-title"><?= $usermember['mbrtitle'] ?></span></td></tr>
	  ...
	  <div class="row">
		...
		<div class="row">
        	...
        	['id' => 'save-' . $userbotModel->formName(),  'class' => 'btn btn-success']); ?>
				...
				<?= Html::a(Yii::t('app', 'Active subscriptions'), /*Yii::$app->request->getReferrer()*/ ['/membership/index'], ['class'=>'btn btn-primary ml-1']) ?>
			</div>
		</div>
  	<?php ActiveForm::end(); ?>
	</div>
...
<div class="botsignal">
	...
	<?php if (($maxSignals == 0) || ($usedSignals < $maxSignals)) : ?>
		<?= HTML::a('<i class="fa fa-plus"></i> '.Yii::t('app', 'Add signal'), ['/bot/addbotsignal', 'id'=>$userbotModel->id], ['class'=>'btn btn-primary mt-2'])
	...
	<?php if (!empty($botsignalModel)) : ?>
		...
		<?php $bsgnr=0; foreach($botsignalModel as $bsgid => $botsignal) : $bsgnr++; ?>
			<tr><td><?= $bsgnr ?></td>
				<td>
					<?= HTML::a(/*Yii::t('app', 'Update botsignal')*/'<i class="fa fa-pen-to-square"></i>', ['/bot/updatebotsignal', 'id' => $botsignal->id], ['class'=>'btn btn-info']) ?>
					<?= Html::a('<i class="fa fa-trash"></i>', ['/bot/delbotsignal', 'id'=>$botsignal->id], ['class'=>'btn btn-danger', 'onclick'=>'return confirm("'.Yii::t('app','Are you sure you want to remove this signal?').'")']) ?>
				</td>
