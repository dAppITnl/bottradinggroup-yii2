...
use backend\models\MembershipSearch;
...
class MembershipController extends Controller
{
	...
	*/
    ...
    $model = new Membership;
      ...
      $ok = false;
      if ($model->load($_POST)) {
        $model->mbr_createdat = $model->mbr_updatedat = time();
        $model->mbrusr_created_id = $model->mbrusr_updated_id = \Yii::$app->user->id;
        $model->mbr_createdt = $model->mbr_updatedt = date('Y-m-d H:i:s', time());
	...
	*/
    ...
    try {
      $model = $this->findModel($id);
			$model->mbr_discordroles = explode(',', $model->mbr_discordroles);
      $ok = false;
      if ($model->load($_POST)) {
				$model->mbr_discordroles = implode(',', $model->mbr_discordroles);
        $model->mbr_updatedat = time();
        $model->mbrusr_updated_id = \Yii::$app->user->id;
        $model->mbr_updatedt = date('Y-m-d H:i:s', time());
        if ($model->save()) { $ok = true; }
      }
      if ($ok) {
	...
	*/
    ...
    try {
      ...
      $model->mbr_deletedat = time();
      $model->mbrusr_deleted_id = \Yii::$app->user->id;
      $model->mbr_deletedt = date('Y-m-d H:i:s', time());
      if ($model->save()) { $ok = true; }
    } catch (\Exception $e) {
      $msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
	...
	*/
	protected function findModel($id)
	{
		if (($model = Membership::findOne($id)) !== null) {
			...
			throw new HttpException(404, 'The requested membership does not exist.');
		}
	}
}
